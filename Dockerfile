FROM nvidia/cuda:12.2.0-devel-ubuntu22.04

# 1) Системные зависимости (добавляем jq для парсинга ngrok JSON)
RUN apt-get update && apt-get install -y \
      build-essential git cmake ninja-build wget curl jq \
      python3 python3-pip python3-dev \
      libopenblas-dev libssl-dev zlib1g-dev libcurl4-openssl-dev \
    && rm -rf /var/lib/apt/lists/*

# 2) Собираем llama-server
WORKDIR /app/llama.cpp
COPY llama.cpp ./
RUN cmake -B build -DGGM_CUDA=ON -DLLAMA_CURL=ON . \
 && cmake --build build --parallel 2 --target llama-server

# 3) Python-зависимости и рабочая директория
WORKDIR /app
RUN pip3 install --no-cache-dir llama-cpp-python fastapi uvicorn[standard]

# 4) Папка для модели
RUN mkdir -p /models

# 5) Копируем код и скрипт старта
COPY server.py /app/server.py
COPY entrypoint.sh /entrypoint.sh
RUN chmod +x /entrypoint.sh

EXPOSE 8000
ENTRYPOINT ["/entrypoint.sh"]
